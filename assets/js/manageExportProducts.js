...
function postProducts() {
  ...
  document.querySelector(".btn-add").onclick = function(e) {
    ...
    var dataPost = {
      ...
      totalExport: Number.parseInt(document.querySelector("input[name='totalExport']").value),
      dateExport: document.querySelector("input[name='dateExport']").value,
      priceProduct: Number.parseInt(document.querySelector("input[name='price']").value),
    ...
    getProducts(function (data){
      var presentTotal = data.find(function (item) {
        return item.id === document.querySelector("input[name='idExport']").value;
      }).totalProduct;
      var updateTotalProduct =  {totalProduct: presentTotal - Number.parseInt(document.querySelector("input[name='totalExport']").value)};

      fetch(productsApi + "/" + document.querySelector("input[name='idExport']").value, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
...
function renderProducts(data) {
    ...
    var htmls = data.map((item, index) => {
      ...
      return `<tr style="padding: 10px 0" class="product-${item.idExport}">
                ...
                <td><a href="#" class="btn-delete btn-red" onclick="deleteProducts(${item.idExport})"></i>Xóa</a></td>
              </tr>`;
    });
...
function renderDateProducts(data){
  ...
  var htmls = data.map((item, index) => {
    ...
    if(dateTemp.getDate() === dateSel.getDate() && dateTemp.getMonth() === dateSel.getMonth() && dateTemp.getFullYear() === dateSel.getFullYear()) {
      var totalPrice = item.priceProduct * item.totalExport;
      return `<tr style="padding: 10px 0" class="product-${item.idExport}">
              <td>${index + 1}</td>
              <td>${item.nameProduct}</td>
